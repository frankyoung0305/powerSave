...
int turn_cpu(int type, int cpu) {
	...
	if(type == 1 && cpu >= 1 && cpu < PHYSICAL_CPUS) {//turn on
		char cmd1cstate1[100];
		char cmd1cstate2[100];
		char cmd1cstate3[100];
		char cmd1cstate4[100];
		char cmd2cstate1[100];
		char cmd2cstate2[100];
		char cmd2cstate3[100];
		char cmd2cstate4[100];
		sprintf(cmd1cstate1, "cpupower -c %d idle-set -d 1 ", cpu);
		sprintf(cmd1cstate2, "cpupower -c %d idle-set -d 2 ", cpu);
		sprintf(cmd1cstate3, "cpupower -c %d idle-set -d 3 ", cpu);
		sprintf(cmd1cstate4, "cpupower -c %d idle-set -d 4 ", cpu);
		sprintf(cmd2cstate1, "cpupower -c %d idle-set -d 1 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate2, "cpupower -c %d idle-set -d 2 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate3, "cpupower -c %d idle-set -d 3 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate4, "cpupower -c %d idle-set -d 4 ", cpu + PHYSICAL_CPUS);
		...
		FILE* bash1cstate1 = popen(cmd1cstate1, "r");
		FILE* bash1cstate2 = popen(cmd1cstate2, "r");
		FILE* bash1cstate3 = popen(cmd1cstate3, "r");
		FILE* bash1cstate4 = popen(cmd1cstate4, "r");
		FILE* bash2cstate1 = popen(cmd2cstate1, "r");
		FILE* bash2cstate2 = popen(cmd2cstate2, "r");
		FILE* bash2cstate3 = popen(cmd2cstate3, "r");
		FILE* bash2cstate4 = popen(cmd2cstate4, "r");
		...
		sprintf(cmd1cstate1, "cpupower -c %d idle-set -e 1 ", cpu);
		sprintf(cmd1cstate2, "cpupower -c %d idle-set -e 2 ", cpu);
		sprintf(cmd1cstate3, "cpupower -c %d idle-set -e 3 ", cpu);
		sprintf(cmd1cstate4, "cpupower -c %d idle-set -e 4 ", cpu);
		sprintf(cmd2cstate1, "cpupower -c %d idle-set -e 1 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate2, "cpupower -c %d idle-set -e 2 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate3, "cpupower -c %d idle-set -e 3 ", cpu + PHYSICAL_CPUS);
		sprintf(cmd2cstate4, "cpupower -c %d idle-set -e 4 ", cpu + PHYSICAL_CPUS);
...
struct check_report check_status(struct record * statistics[PROC_NUMBER], double * point_weight) {
	...
	if(count_idle_cpu <= 1) {//not enough idle CPUs.
		...
		report.type = 2;
		for(i = FIRST_WORKING_CPU, j = 0;i < PHYSICAL_CPUS;i++) {//i from 1 to PHYSICAL_CPUS - 1.
			if(old_cpu_status[i] >= CPU_IDLE_TIMES) {
				report.cpus[j] = i;
				j++;
				old_cpu_status[i] = 0;
