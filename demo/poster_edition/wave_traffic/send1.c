...
int main() {
	...
	int sleep_freq = SEND_SLEEP_FREQUENCY;
////////////////wave
...
*/
		if(i % TIME_CHECK_FREQ == 0){
			if(gettimeofday(&time, NULL) != 0) {
				printf("gettimeofday for now failed \n");
				return 1;
			}
			//printf("now time %d \n", time.tv_sec);
			if(time.tv_usec - last_time.tv_usec + (time.tv_sec - last_time.tv_sec) * 1000000 >= FREQ_CHANGE_SPAN){
				sleep_freq = genSleepFreq(wave);
				printf("SLEEP FREQ = %d \n", sleep_freq);	
				wave++;				
				last_time = time;	
			}
