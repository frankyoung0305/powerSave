...
#define POINT_WEIGHT_ALPHA 0.3//point_weight = alpha * point_weight_old + (1 - alpha) * p_cpu_usage.
...
void show_working_cpu(struct record * statistics[PROS_NUMBER]);//show which CPU processes are working on
...
void controller_control(union sigval sv) {
	...
	static int p_report_status[PROS_NUMBER] = {0, 0, 0};
	...
	switch(ctrlbuffer.service_number)
	{
		case 2://the answer message about process sent by processes.
			...
			if(update_status == 1) {
				...
				clear_int_series(PROS_NUMBER, p_report_status, "p_report_status");
...
//*************************update record************************************
			...
			if(idle_cpu != -1) {
				...
				int p_on_hot_cpu[PROS_NUMBER];//store processes who work on the hot CPU.
...
void show_working_cpu(struct record * statistics[PROS_NUMBER]) {
	int i = 0;
	for(i = 0; i < PROS_NUMBER; i++) {
...
void update_point_weight(struct record * statistics[PROS_NUMBER], double * point_weight) {
	int i = 0;
	for(i = 0;i < PROS_NUMBER;i++) {
		point_weight[i] = statistics[i]->cpu_usage = get_cpu_usage(statistics[i]->pid_in_record);
	}
