...
//*************************update record************************************
	...
	printf("\nFINISH A CONTROLLER CONTROL!!!! ctrlbuffer.service_number = %ld \n\n", ctrlbuffer.service_number);
	pthread_exit(NULL);
}
...
struct check_report check_status(struct record * statistics[PROC_NUMBER], double * point_weight) {
	...
	for(i = 2;i < PHYSICAL_CPUS;i++) {////i from 2 to PHYSICAL_CPUS.CPU0 is only for PACKET_SENDING and controller.
		if(p_on_cpu[i] > 0) {
	...
	if(count_idle_cpu <= 1) {
		...
		report.type = 2;
		for(i = 1, j = 0;i < PHYSICAL_CPUS;i++) {//i from 1 to PHYSICAL_CPUS - 1.
			if(old_cpu_status[i] >= CPU_IDLE_TIMES) {
				report.cpus[j] = i;
				j++;
				old_cpu_status[i] = 0;
