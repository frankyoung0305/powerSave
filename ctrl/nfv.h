...
#define PERM 0766
#define PACKETS 10000000//the number of packets which we will send.
...
struct ctrlmsg{
	long service_number;/*	1:the controller asks processes to report message about themselves.
				...
				4:the processes send to controller to tell that the process has finished all work and controller could free the queues between them. */
...
struct sta checkcpu();//check which CPUs the process can run on.
