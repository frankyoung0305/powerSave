...
struct rank_ans rank(int ADJ_SIZE, int MAX_LOOP_TIMES);
...
struct rank_ans rank(int ADJ_SIZE, int MAX_LOOP_TIMES) {
	...
	KL_re = KL_step(MAX_LOOP_TIMES, new_edge, adj_array, point_weight, set_edge, set1, set2);
	gettimeofday(&end, 0);
	...
	rank_re.loop_times = KL_re.loop_times;
	return rank_re;
