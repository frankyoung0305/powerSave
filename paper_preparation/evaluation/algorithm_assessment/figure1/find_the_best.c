...
int main(void) {
	...
	double raw_adj_array0[ADJ_SIZE][ADJ_SIZE] = {
												{0,	90,	30,	70,	0,	0},
												{0,	0,	0,	30,	0,	0},
												{0,	0,	0,	20,	40,	0},
												{0, 0,	0,	0,	15,	65},
	...
	double raw_point_weight0[ADJ_SIZE] = {0.12, 0.15, 0.25, 0.30, 0.09, 0};
	...
	do{
		...
		if(gain > 0) {
			//if(best.edge_cut > re.edge_cut && best.workload_diff > re.workload_diff) {
			if(gain > gain_max) {
				gain_max = gain;
				best = re;
